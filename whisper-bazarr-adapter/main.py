"""Bazarr <-> faster-whisper adapter.

Bazarr only understands the legacy ahmetoner/whisper-asr-webservice API
(`POST /asr`, `POST /detect-language`), while the production ASR engine is a
faster-whisper server (fedirz) exposing the OpenAI-compatible
`/v1/audio/transcriptions` / `/v1/audio/translations` endpoints. This service
implements the ahmetoner surface and forwards to the faster-whisper server.

Why the ffmpeg normalization step?
----------------------------------
faster-whisper's CTranslate2 frontend expects 16 kHz mono PCM. Bazarr *does*
pre-encode to 16 kHz mono WAV, but that is a property of Bazarr, not of the
protocol: any other client (or a Bazarr change) can send AAC/Opus/48 kHz/
stereo/5.1 streams in MP4/MKV/WebM containers that the ASR engine cannot
decode. Normalising here makes the adapter robust to the source codec
regardless of client, and is a no-op when the input is already WAV/16k/mono.
Do not remove it -- it is the safety net for the "hard" codecs.

Why a seekable temp input file (not pure stdin->stdout)?
---------------------------------------------------------
MP4/MKV/WebM containers can store their index/metadata (`moov`/`segment`
header) at the END of the stream, which the demuxer must *seek* back to in
order to decode. A pipe is not seekable, so piping the raw upload into
`ffmpeg -i pipe:0` fails for exactly these containers ("Invalid data found ...
partial file") and yields an empty WAV. We therefore write the upload to a
seekable temp file (keeping its extension so ffmpeg picks the right demuxer)
